...
MAX_RUNTIME_SECONDS = 20700  # 5 hours, 45 minutes
START_TIME = time.time()

# Global database variable so the Signal Handler can access and save it in an emergency
global_db = {}
...
# --- INITIALIZATION (Fixes the git crash) ---
def init_files():
    """Ensures files exist immediately so GitHub doesn't crash if cancelled instantly."""
...
def request_shutdown(signum, frame):
    ...
    sys.exit(0) # Exit cleanly to allow git commit
...
# --- HTML PROCESSOR (Fixes missing metrics) ---
def get_clean_text(html_string):
    """Converts break tags to newlines before stripping HTML so lines don't mash together."""
...
def extract_source_data(html_content, review_url, category):
    ...
    for img in entry_content.find_all('img'):
        ...
        if not raw_data["Bias"]:
            if "satire" in alt: raw_data["Bias"] = "Satire"
            ...
            elif "left center bias" in alt: raw_data["Bias"] = "Left-Center"
            elif "right center bias" in alt: raw_data["Bias"] = "Right-Center"
...
def main():
    ...
    for url, cat_name in CATEGORIES.items():
        ...
        try:
            ...
            print(f"  -> {cat_name}: Found {len(raw_links)} links. {len(pending_tasks[cat_name])} new pending.", flush=True)
    ...
    for cat_name, urls in sorted_tasks:
        ...
        for idx, href in enumerate(urls, 1):
            ...
            try:
                ...
                if urls_processed_this_run % 5 == 0:
                    save_db(global_db)
