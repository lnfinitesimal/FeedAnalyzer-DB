...
# FIX 2: Signal handler — each save isolated so one failing can't block the other
def request_shutdown(signum, frame):
    global SHUTDOWN_REQUESTED
    if not SHUTDOWN_REQUESTED:
        SHUTDOWN_REQUESTED = True
        print(f"\n[!] Shutdown signal received (signal {signum}). Emergency save...", flush=True)
        try:
            save_db(global_db)
            print("[!] Emergency DB save OK.", flush=True)
        except Exception as e:
            print(f"[!] Emergency DB save failed: {e}", flush=True)
            traceback.print_exc()
        try:
            generate_statistics(global_db, master_totals)
            print("[!] Emergency stats save OK.", flush=True)
        except Exception as e:
            print(f"[!] Emergency stats save failed: {e}", flush=True)
            traceback.print_exc()
...
def generate_statistics(db, stats_totals=None):
    ...
    try:
        ...
        # FIX 1: Added traceback so you can see exactly what line fails
...
def main():
    ...
    try:
        ...
        for cat_name, urls in execution_groups.items():
            ...
            for idx, href in enumerate(urls, 1):
                ...
                try:
                    ...
                    # Save DB after every entry
                    save_db(global_db)
                    # FIX 4: Save stats after every entry too — always in sync with DB
                    generate_statistics(global_db, master_totals)
        ...
        # FIX 3: Each save isolated — one failing cannot block the other
        print("\n[OK] Processing complete. Final save...", flush=True)

        try:
            save_db(global_db)
        except Exception as e:
            print(f"[!] Final save_db error: {e}", flush=True)
            traceback.print_exc()

        try:
            generate_statistics(global_db, master_totals)
        except Exception as e:
            print(f"[!] Final generate_statistics error: {e}", flush=True)
            traceback.print_exc()
